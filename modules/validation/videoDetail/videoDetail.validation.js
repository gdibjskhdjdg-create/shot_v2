...
const { VideoDetailStatus_Enum, findVideoDetailStatusByValue } = require("../../models/videoDetail/enum/VideoDetail.enum");

class VideoDetailValidation extends Validation {

    async updateVideoDetail(data = {}) {
        this.setEmpty()
        const {
            title,
            description,
    ...
    async updateCleaningVideoDetail(data = {}) {
        this.setEmpty()
        const {
            status,
            cleaningDescription,
        ...
        /* validate state */
        if (![VideoDetailStatus_Enum.accept.value, VideoDetailStatus_Enum.reject.value].includes(status)) {
    ...
    async createSectionTimeOfVideoDetail(data = {}) {
        this.setEmpty();
        const {
            title,
            startTime,
    ...
    checkStatus(status) {
        ...
        if (!findVideoDetailStatusByValue(status)) {
            this.setError("videoDetail status is Invalid");
        }
        this.setValidData("status", status);
        return this.getResult();
    ...
    checkQueryStatusIsBySourceStatus(sourceStatus, queryStatus) {
        ...
        if (!queryStatus) {
            ...
            if (sourceStatus == VideoDetailStatus_Enum.init.value) {
                validQueryStatus.push(VideoDetailStatus_Enum.cleaning.value)
            } else if (sourceStatus == VideoDetailStatus_Enum.cleaning.value) {
                validQueryStatus.push(VideoDetailStatus_Enum.accept.value)
                validQueryStatus.push(VideoDetailStatus_Enum.reject.value)
            }
    ...
    updateStatus(body) {
        ...
        const validStatus = Object.keys(VideoDetailStatus_Enum).map((key) => {
            return VideoDetailStatus_Enum[key].value;
        })
        ...
        if (status == VideoDetailStatus_Enum.reject.value) {
    ...
    exportVideoDetails(videoDetailsId) {
        this.setEmpty();
...
module.exports = new VideoDetailValidation();