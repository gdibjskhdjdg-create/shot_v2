...
const listTags = async (req, res) => {
    ...
    const tags = await TagService.fetch({ search, categoryId, excludeId, type, page, take, shotUsageCount: true, sortKey, sortACS });
...
const searchTags = async (req, res) => {
    ...
    const tags = await TagService.fetch({ search, excludeId, type, page, take });
    return BaseController.ok(res, { tags: Tag_DTO.create(tags.rows), count: tags.count });
}

const getTagDetails = async (req, res) => {
    const { tagId } = req.params;
    const tag = await TagService.fetchDetails(tagId);
...
const getTagShots = async (req, res) => {
    ...
    const tag = await TagService.fetchShots(tagId, query);
    return BaseController.ok(res, tag);
}
...
const removeShotFromTag = async (req, res) => {
    ...
    await TagService.unlinkShot(tagId, shotId)
