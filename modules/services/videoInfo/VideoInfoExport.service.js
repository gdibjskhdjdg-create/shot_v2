...
class VideoInfoExportService extends Service {
    ...
    async getSpecialList(filters = {}) {
        const { videoDetails, count } = await this.VideoInfoService.specialList(filters);
    ...
    async getExportVideoDetailsId(videoFileId, isExcludeMode, filters = {}) {
        ...
        if (isExcludeMode) {
            const response = (await this.getSpecialList({ excludesId: videoFileId, ...filters, page: 1, take: null }))
    ...
    async exportSpecial(exportType, filters = {}) {
        const { videoDetails, count } = await this.getSpecialList(filters);
        return await this.export(exportType, videoDetails.map(x => x.videoFileId))
    }

    async export(exportType, videoFileId) {
        ...
        let videoDetails = await this.VideoInfoService.detail(videoFileId);
        videoDetails = await this.prepareToExport(videoDetails);
    ...
    async prepareToExport(videoDetails) {
        const defaultValues = await this.VideoInfoService.getBasicInfo();

        const videoDetailsScore = (await this.VideoInfoScoreService.getBySection({ videoFileId: videoDetails.map(x => x.videoFileId) })).map(x => x.toJSON())
        ...
        for (const videoDetail of videoDetails) {
            ...
            const scoreKeys = VideoInfoScoreEntity.getAllKeys()
            videoDetail.score = []
    ...
    async excelExport(videoDetails = []) {
        const header = [
            ...
            { value: 'نریتور' },
            ...VideoInfoScoreEntity.getAllTitles().map(value => ({ value })),
        ];
    ...
    async exportSpecialPath(exportType, filters = {}) {
...
module.exports = VideoInfoExportService;
