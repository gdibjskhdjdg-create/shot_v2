...
const { takeScreenShot } = require('../FFmpeg/FFmpeg.service');
const GalleryParser = require('../../../helper/galleryParser.tool');
const emitter = require('../../_default/eventEmitter');
const { VideoInfoStatus_Enum, VideoInfoShotStatus_Enum } = require('../../models/videoDetail/enum/VideoInfo.enum');
const TableCountService = require('../tableCount/TableCount.service');

class VideoInfoService extends Service {
    constructor(videoFileService = () => { }, shotService = () => { }, VideoInfoScoreService = () => { }) {
        super(VideoDetail);

        this.VideoInfoScoreService = VideoInfoScoreService;
        this.videoFileService = videoFileService;
        this.shotService = shotService
        this.pathToRoot = path.join(__dirname, '..', '..', '..');

        this.videoDetailList = this.list.bind(this);
        this.updateMainInfo = this.updateMainInfo.bind(this);
    }

    async findOrCreate(videoFile) {
        ...
        if (!detail) {
            ...
            detail = await VideoDetail.create({
                ...
                status: VideoInfoStatus_Enum.init.value,
                aiTagStatus,
                shotStatus: VideoInfoShotStatus_Enum.initCheck.value
            })
        }
    ...
    async checkCanUpdateInit(videoFileId) {
        const response = await VideoDetail.findOne({ where: { videoFileId } });
        return response && [VideoInfoStatus_Enum.init.value, VideoInfoStatus_Enum.cleaning.value].includes(response.status);
    }
    ...
    async updateMainInfo(videoFileId, data, otherInfo = {}) {
    ...
    async initToCleaning(videoFileId, userId, data) {
        const response = await this.updateWithStatus(videoFileId, userId, {
            ...data,
            status: VideoInfoStatus_Enum.cleaning.value,
            logStatus: VideoInfoStatus_Enum.init.value
        });

        return response;
    }

    async cleaning(videoFileId, userId, data) {
        const response = await this.updateWithStatus(videoFileId, userId, {
            ...data,
            logStatus: VideoInfoStatus_Enum.cleaning.value
        });

        return response;
    }

    async updateWithStatus(videoFileId, userId, data) {
        const response = await this.update(videoFileId, userId, data);

        emitter.emit("updateVideoInfoStatus", {
            videoFileId,
            userId: userId,
            body: {
                mode: data.logStatus,
    ...
    async update(videoFileId, userId, data) {
        const videoDetail = await this.updateMainInfo(videoFileId, data);
    ...
    async deleteVideoInfosOfProject(projectId) {
    ...
    async deleteVideoInfoOfVideoFile(videoFileId) {
        await VideoDetail.destroy({ where: { videoFileId } });
        // emitter.emit('deleteVideoDetailsByProject', videoFileId)
        return;
    }

    // async createVideoInfoForVideoFile(videoFileId, userId, params) {
    ...
    //     try {
    //         await this.update(newVideoDetail.videoFileId, userId, data);
    ...
    async newVideoFile(video) {
        ...
        const filters = {
            ...
            aiTagStatus : isAI? 'queue': null,
            status: VideoInfoStatus_Enum.init.value
        }

        return await VideoDetail.create(filters)
    ...
    async aiKeywordsList(query = { page: 1, take: 10, title }) {
    ...
    async aiKeywordsReport(videoFileId) {
    ...
    async firstAIByStatus(status = 'queue', withRelation = true) {
    ...
    async checkFirstQueueForAI() {
        ...
        return await this.firstAIByStatus('queue')
    }

    async checkFirstPendingForAI(withRelation = true) {
        return await this.firstAIByStatus('pending', withRelation)
    }

    async makeFromVideo(video, status = VideoInfoStatus_Enum.accept.value) {
    ...
    async updateStatus({ videoFileId, status = null, cleaningDescription = null }) {
        ...
        let shotStatus = VideoInfoShotStatus_Enum.initCheck.value;
        if (shotVideoStatus.length) {
            if (shotVideoStatus.includes(VideoInfoShotStatus_Enum.initCheck.value)) {
                shotStatus = VideoInfoShotStatus_Enum.initCheck.value;
            } else if (shotVideoStatus.includes(VideoInfoShotStatus_Enum.editor.value)) {
                shotStatus = VideoInfoShotStatus_Enum.editor.value;
            } else if (shotVideoStatus.includes(VideoInfoShotStatus_Enum.equalizing.value)) {
                shotStatus = VideoInfoShotStatus_Enum.equalizing.value;
            } else {
                shotStatus = VideoInfoShotStatus_Enum.equalized.value;
            }
        }
    ...
    async updateRelTables(videoDetail, params) {
        ...
        await this.updateCategories(videoDetail.videoFileId, data);
        await this.updateKeyword(videoDetail.videoFileId, data);
        await this.updateKeywordInVideo(videoDetail.videoFileId, data);
        videoDetail = await this.updateLanguage(videoDetail, data);
        videoDetail = await this.updateGallery(videoDetail, data);
    }

    async updateGallery(videoDetail, data) {
    ...
    async updateLanguage(videoDetail, data, otherInfo = {}) {
    ...
    async updateCategories(videoFileId, data = {}, more = {}) {
    ...
    async updateKeyword(videoFileId, data = {}, more = {}) {
    ...
    async updateKeywordInVideo(videoFileId, data, more = {}) {
    ...
    async updateStatus_Service(videoFileId, isExcludeMode, filters = {}, status) {
        let videoDetailsId = [];
        if (isExcludeMode) {
            let { videoDetails, count } = await this.specialList({ ...filters, excludesId: videoFileId });
            videoDetailsId = videoDetails.map(item => item.videoFileId);
        }
        else {
            let { videoDetails, count } = await this.specialList({ id: videoFileId });
            videoDetailsId = videoDetails.map(item => item.videoFileId);
        }
    ...
    async updateScores_Service(userId, videoFileId, isExcludeMode, filters = {}, scores) {
        let videoDetailsId = [];
        if (isExcludeMode) {
            let { videoDetails, count } = await this.specialList({ ...filters, excludesId: videoFileId });
            videoDetailsId = videoDetails.map(item => item.videoFileId);
        ...
        await this.VideoInfoScoreService.storeScoreGroup(userId, videoDetailsId, scores)
    ...
    async getExportInfoVideos({ videos: videosId, isExcludeMode, filters = {} }) {
        ...
        let videosDetails;
        if (isExcludeMode) {
            videosDetails = (await this.specialList({ ...filters, excludesId: videosId, page: 1, take: null }))?.videoDetails || []
        } else {
    ...
    async getVideoListWithCode(UUID, newFilters = {}) {
        ...
        return this.specialList({...filters, page, take })
    }
    ...
    async validateCodeWithVideoId(videoDetailId, UUID) {
        ...
        let check = await this.specialList({...filters, id: videoDetailId });
