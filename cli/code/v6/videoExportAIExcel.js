...
const { Category, VideoDetailRelCategory, sequelize, ShotInput } = require("../../../modules/_default/model");
...
(async () => {
    try {
        ...
        for (let i = 0; i < categories.length; i++) {
            ...
            const videosOfCategoryQuery = "SELECT DISTINCT(videoFileId ) as videoFileId   FROM `video_detail_category` where categoryId=:categoryId;"
            let videosOfCategory = (await sequelize.query(videosOfCategoryQuery, { replacements: { categoryId: category.id }, type: QueryTypes.SELECT })).map(x => x.videoFileId);
            ...
            while (true) {
                let videos = videosOfCategory.slice(index, count)
                if (videos.length == 0) break

                // console.log(videos.join(","))
                const tagsQuery = "select st.tagId as s_tagId, tt.tag as tag_name, si.title as si_title, si.id as sid, COUNT(st.videoFileId) as scount, st.videoFileId  as st_videoFileId  from `video_detail_tag` st left join `shot_inputs` si on st.inputId = si.id left join `tags` tt on st.tagId = tt.id where st.videoFileId  IN (:videos) group by si_title, sid, s_tagId"
                let tags = await sequelize.query(tagsQuery, { replacements: { videos }, type: QueryTypes.SELECT })
        ...
        for (let i = 0; i < categoryItems.length; i++) {
            ...
            for (let i = 0; i < tags.length; i++) {
                let { s_tagId: tag_id, tag_name: tag_title, sid: input_id, input_title, scount: video_count } = tags[i]
        ...
        await writeXlsxFile(
            fullData,
            {
                schema: fullSchema,
                sheets: sheetsNames,
                filePath: path.join(__dirname, "video_tags.xlsx")
            }
        )
    }
    catch (err) {
